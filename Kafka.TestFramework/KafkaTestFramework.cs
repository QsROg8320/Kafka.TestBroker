...
namespace Kafka.TestFramework
{
    public abstract class KafkaTestFramework
    {
        ...
        private static readonly ILogger Logger = NullLoggerFactory.Instance.CreateLogger<KafkaTestFramework>();
            ...
            var task = Task.Run(
                async () =>
                {
                    ...
                    while (_cancellationTokenSource.IsCancellationRequested == false)
                    {
                        ...
                        {
                            Logger.LogDebug(ex, "Client disconnected or network error, ending message loop for this client");
                            return;
                        }
                    }
                });
