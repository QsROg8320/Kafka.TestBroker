...
namespace Kafka.TestFramework
{
    internal class SocketServer : INetworkServer, IAsyncDisposable
    {
        ...
        private static readonly ILogger Logger = NullLoggerFactory.Instance.CreateLogger<SocketServer>();
            ...
            _acceptingClientsBackgroundTask = Task.Run(async () =>
            {
                while (_cancellationSource.IsCancellationRequested == false)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        {
                            Logger.LogDebug("Server is shutting down, client accepting loop is stopping.");
                            return;
                        }

                        Logger.LogError(ex, "An error occurred while accepting a new client. The server will continue to listen.");
                    }
                }
            });
        }
